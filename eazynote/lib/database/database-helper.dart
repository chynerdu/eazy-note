...
class DatabaseHelper {
  ...
  //   Directory documentsDirectory = await getApplicationDocumentsDirectory();
  //   String path = join(documentsDirectory.path, _databaseName);
  //   var ourDb = await openDatabase(path, version: _databaseVersion,
  //   onCreate: _onCreate,
  ...
  closeDatabase(Database database) async {
  ...
  Future<List> queryAllRows() async {
    Database db = await instance.database;
    var listResult = await db.query(table);
    print('result here $listResult');
    // final List<NotesModel> fetchedNoteList = [];
    // listResult.forEach((dynamic noteList) {
